...
namespace Titan {
	...
	void TTN_BloomEffect::ApplyEffect(TTN_PostEffect::spostptr buffer)
	{
		...
		for (int i = 0; i < m_numOfPasses; i++) {
			...
			//set the uniforms
			m_shaders[shaderIndex]->SetUniform("u_Step", m_radius / m_buffers[0]->m_width);
			m_shaders[shaderIndex]->SetUniform("u_Strength", m_strength);
			m_shaders[shaderIndex]->SetUniform("u_Weights", m_weights[0], 5);
			...
			//sets the uniforms
			m_shaders[shaderIndex + 1]->SetUniform("u_Step", m_radius / m_buffers[0]->m_height);
			m_shaders[shaderIndex + 1]->SetUniform("u_Strength", m_strength);
			m_shaders[shaderIndex + 1]->SetUniform("u_Weights", m_weights[0], 5);
			//renders to the fullscreen quad in the second (hori blur) framebuffer
			m_buffers[1]->RenderToFSQ();
			//unbinds everything
